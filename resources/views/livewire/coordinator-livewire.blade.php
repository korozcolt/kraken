<div wire:init="loadCoordinator">
                ...
                <section class="container mx-auto p-6">
                    ...
                    <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
                        <div class="w-full overflow-x-auto">
                            ...
                            @endif
                            @if( count($coordinators) && (auth()->user()->role == 5 || auth()->user()->role == 6 || auth()->user()->role == 2 || auth()->user()->role == 9))
                                <table class="w-full">
                                    <thead>
                                    ...
                                    @foreach($coordinators as $value)
                                        <tr class="text-gray-700">
                                            ...
                                            <td class="px-4 py-3 text-xs border text-center flex">
                                                ...
                                                <a href="#" class="text-gray-400 hover:text-gray-100 ml-2" wire:click="$emit('deleteCoordinator',{{ $value->id }})">
    ...
    <x-jet-dialog-modal wire:model="open_edit">
        <x-slot name="title">
            Edita un registro de Coordinador {{ $coordinator->name }} {{ $coordinator->last }}
        </x-slot>
        <x-slot name="content">
            <div class="mb-4">
                <x-jet-label value="Nombre" class="text-left"></x-jet-label>
                <x-jet-input type="text" class="w-full" wire:model="coordinator.name"></x-jet-input>
            </div>
            <div class="mb-4">
                <x-jet-label value="Apellido" class="text-left"></x-jet-label>
                <x-jet-input type="text" class="w-full" wire:model="coordinator.last"></x-jet-input>
            </div>
            <div class="mb-4">
